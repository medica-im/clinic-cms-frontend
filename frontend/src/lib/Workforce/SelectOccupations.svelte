...
<Select {label} {itemId} items={$occupations} multiple on:change={handleChange} on:clear={handleClear} placeholder="{$LL.ADDRESSBOOK.OCCUPATIONS.PLACEHOLDER()}" bind:value {searchable} />
</div>
{/await}
