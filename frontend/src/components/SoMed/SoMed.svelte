...
{#if appBar}
{#each data as somed}
<a href={somed.url} title={somed.get_type_display} class="btn-icon btn-icon-sm hover:variant-soft-secondary" target="_blank" rel="noreferrer">
...
{:else if appRail}
{#each data as somed}
<AppRailTile label={somed.get_type_display} tag="a" href={somed.url} value={'blog'} on:click={onListItemClick} class="lg:hidden">
    <span><Fa icon={getIcon(somed)} size="lg" /></span>
</AppRailTile>
{/each}
{:else}
{#each data as somed}
<a href={somed.url} class="btn variant-soft-secondary" target="_blank" rel="noreferrer">
    <span ><Fa icon={getIcon(somed)} size="lg" /></span>
</a>
{/each}
{/if}